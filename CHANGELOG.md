...
### Added
- **RuvSense Edge professionalization metrics and config validation.** `ruvsense-master` now derives `schemars` JSON Schemas for runtime, swarm, training, HOMECORE automation, and BFLD blueprint config shapes; accepts explicit startup config via `--config-file` / `RUVSENSE_CONFIG_FILE` (JSON/TOML/YAML); validates checked-in TOML/YAML under `v2/` with `--validate-config-root`; and exposes runtime schema at `/api/v1/config/schema`. `/metrics` now renders through a Prometheus registry with per-node FPS, UDP drop/reorder counters, active tracking zones, DSP and neural latency gauges, and breathing/heartbeat tensor compression ratios. `LOG_FORMAT=json|text` selects structured JSON or default text tracing logs. ADR-135 calibration drift detection is now available as a recommendation-only rolling detector with 300-frame windows, threshold `> 4.0`, and three-window confirmation.
- **RuvSense Edge production documentation refresh.** README, user guide, and agent instructions now describe the live-only appliance behavior: no automatic demo/mock/simulation fallback, `ruvsense-master` + RuvSense Console as the Pi production path, Docker Pi defaults with `CSI_SOURCE=esp32`, `RUVSENSE_MIN_NODES=3`, at least three ESP32-C6 nodes, two environment-configurable mesh APs, and simulation only as an explicit development/test mode.
- **RuvSense Edge appliance refonte.** The former Rust sensing server is now packaged as `ruvsense-master` with Raspberry Pi 4 oriented runtime defaults: UDP/5005 CSI ingest, HTTP/WS on 3000/3001, SQLite runtime state under `/var/lib/ruvsense`, readiness gated by `RUVSENSE_MIN_NODES=3`, Prometheus `/metrics`, and stable `/api/v1/fleet`, `/api/v1/nodes`, `/api/v1/environment`, `/api/v1/modules`, and `/api/v1/calibration` surfaces. Added `docker/Dockerfile.master`, the single `docker/compose.yml`, C6 fleet provisioning (`provision_three_c6.py`), and the simplified RuvSense Console with setup checklist, AP mesh, fleet, modules, calibration, logs, and advanced diagnostics links.
