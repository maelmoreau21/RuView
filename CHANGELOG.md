...
### Added
- **RuvSense Edge professionalization metrics and config validation.** `ruvsense-master` now derives a `schemars` JSON Schema for runtime config, rejects malformed/unknown/semantically invalid config files on startup, persists config through a temp-file/sync/rename flow, and expands `/metrics` with CSI frame counters, UDP drop ratios, reordering counters, jitter buffer gauges, pose processing latency, and active tracking zones. ADR-135 calibration drift detection is now available as a recommendation-only rolling detector with 300-frame windows, threshold `> 4.0`, and three-window confirmation.
